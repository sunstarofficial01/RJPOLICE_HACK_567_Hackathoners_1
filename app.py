...
import librosa
import os
os.chdir('C:\\Users\\hp\\OneDrive\\Documents\\deep fake source\\deepfake audio\\Audio-Deepfake-Detection-main\\Final\\')
import time
...
def model():
    ...
    if request.method == 'POST':
        ...
        if len(features) != num_features:
            error_message = f"Number of features extracted ({len(features)}) does not match the expected number of features ({num_features})."
            return render_template('model.html', error_message=error_message, loader_visible=loader_visible)
        ...
        print("Closest Match Index:", closest_match_idx)
        print("Closest Match Label:", closest_match_label)
        print("Closest Match Probability:", closest_match_prob_percentage)
